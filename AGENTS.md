...
- **Backups share the migration bundle format** (`recovery.py`): `user`/`pre_restore` marker dirs live under `data_dir\backups\<kind>-<stamp>-<uuid>`; user snapshots are never auto-pruned and only they can be deleted. A restore is staged as `data_dir\restore-request.json` and consumed at the next startup *before* the database opens; a failed consume quarantines the request to `restore-request-failed.json` and keeps the current data. Restores swap the database and settings as one transaction and roll both back on any failure: the rollback removes only what it installed and restores only what it saved, so an abort at any step (including the pre-restore snapshot, which fails closed) never deletes untouched live data. Bundles are checked for schema compatibility before the swap — newer or manifest-mismatched schemas are refused. `--backup-now`/`--restore-from` are the headless CLI; the windowed exe has no stdout, so the release E2E globs the exported zip instead of parsing output.
- **Cached content hashes are only trusted with a matching fingerprint**: `files`/`inbox` `content_sha256` must be paired with `hash_fingerprint` (size:mtime_ns); always refresh both together or duplicate detection will match stale content.
- **Replace-previous-version matches by subject+kind, never by filename**; adopted documents can be versioned too, and undo must restore the previous version to the version event's `source_path`, not the filing destination. When a replacement undo is interrupted, reconciliation completes the linked version event's catalogue path (`finish_version_undo`) once the file sits back at its original name.
